...
library("xlsx")
sitenames<-read.xlsx("sites-selected-56.xlsx", sheetName = "Sheet1")
...
for(i in 1:length(sitenames$SITE_ID)){
  flux_path<-paste("flux2015-63sites-process\\",sitenames$SITE_ID[i],sep="")
  flux_name<-paste(flux_path,"\\",sitenames$SITE_ID[i],"_flux2015_modis_daily.csv",sep="")
  print(flux_name)
  ...
  df2<-data.frame(calendar_date=df$calendar_date,gpp_obs=df$GPP_DT_VUT_REF,gpp_mod=df$GPP)
  ...
  write_name<-paste(flux_path,"\\",sitenames$SITE_ID[i],"_model_gpp.csv",sep="")
...
names(df_metrics)<-c("site","PFT","Lat","Lon","model","R2","RMSE","RPE")
View(df_metrics)
write.csv(df_metrics,"result\\sites\\sites_sta.csv")
